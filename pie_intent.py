...
class PIEIntent(object):
    ...
    def get_path(self,
        ...
        if type_save == 'models':
            save_path = os.path.join(root, 'pie', 'intention',input_type, models_save_folder)
        ...
        for seq, pid in zip(img_sequences, ped_ids):
            ...
            for imp, b, p in zip(seq, bbox_sequences[i], pid):
                ...
                if os.path.exists(img_save_path) and not regen_pkl:
                    ...
                    if crop_type=='content':
                        bbox = jitter_bbox(imp, [b],'enlarge', 2)[0] #扩大裁减范围
                    else:
                        bbox = jitter_bbox(imp, [b],'same', 2)[0] #不扩大裁减范围
    ...
    def get_tracks(self, dataset, data_type, seq_length, overlap):
        ...
        for k in data_type.keys():
            d_types.extend(data_type[k])  #d_types=['bbox','intention_binary']
        d = {}
    ...
    def concat_data(self, data, data_type):
        ...
        """
        if not data_type: # when data_type is empty, not data_type is True
            return []
    ...
    def get_test_data(self, data, train_params,data_params, seq_length, overlap):
        """
        ...
        test_img = self.load_images_and_process(images,
                                                ...
                                                ped_ids,
                                                data_type='test',
                                                crop_type=data_params['crop_type'],
                                                save_path=self.get_path(type_save='data',
                                                                        data_type='features_'+data_params['crop_type']+'_pad_resize',  # images
    ...
    def create_lstm_model(self,
                          ...
                          r_state=True,
                          r_sequence=False):
        return LSTM(units=self._num_hidden_units, #128
    ...
    def pie_convlstm_encdec(self):
        ...
        encoder_input=input_data=Input(shape=(self._sequence_length,) + self.context_model.output_shape[1:],
                                       name = "encoder_input") #(none,15,7,7,512)
        ...
        encoder_model = ConvLSTM2D(filters=self._convlstm_num_filters,
                                   ...
                                   dropout=self._lstm_dropout,
                                   recurrent_dropout=self._lstm_recurrent_dropout,
                                   return_sequences=False)(interm_input) #(none,6,6,64)
        encoder_output = Flatten(name='encoder_flatten')(encoder_model) #(none,2304)

        # Generate Decoder LSTM unit
        decoder_input = Input(shape=(self._decoder_seq_length,
                                     self._decoder_input_size),
                              name='decoder_input') #(none,15,4)
        encoder_vec = RepeatVector(self._decoder_seq_length)(encoder_output) #(none,15,2304)
        decoder_concat_inputs = Concatenate(axis=2)([encoder_vec, decoder_input]) #(none,15,2308)

        decoder_model = self.create_lstm_model(name='decoder_network',
                                               r_state = False,
                                               r_sequence=False)(decoder_concat_inputs) #(none,128)

        decoder_dense_output = Dense(self._decoder_dense_output_size,
                                     activation='sigmoid',
                                     name='decoder_dense')(decoder_model) #(none,1)

        decoder_output = decoder_dense_output #(none,1)
    ...
    def train(self,
        ...
        self._encoder_seq_length = train_d['decoder_input'].shape[1] #15
        self._decoder_seq_length = train_d['decoder_input'].shape[1] #15

        self._sequence_length = self._encoder_seq_length #15
    ...
    def test_chunk(self,
            ...
            #因内存不足，把测试数据分为3组
            num_group=15
            num_samples = int(len(data_test['image'])/num_group)
            ...
            for j in range(0,num_group):
                for i in range(0, num_samples, 100):
                    ...
                    data_test_chunk = {}
                    data_test_chunk['intention_binary'] = data_test['intention_binary'][i+j*num_samples:min(i+j*num_samples+100, (j+1)*num_samples)]
                    data_test_chunk['image'] = data_test['image'][i+j*num_samples:min(i+j*num_samples+100,(j+1)*num_samples)]
                    data_test_chunk['ped_id'] = data_test['ped_id'][i+j*num_samples:min(i+j*num_samples+100,(j+1)*num_samples)]
                    data_test_chunk['intention_prob'] = data_test['intention_prob'][i+j*num_samples:min(i+j*num_samples+100,(j+1)*num_samples)]
                    data_test_chunk['bbox'] = data_test['bbox'][i+j*num_samples:min(i+j*num_samples+100,(j+1)*num_samples)]
                ...
                del data_test_chunk
                del test_results_chunk
                del tracks, images_chunk, bboxes_chunk, ped_ids_chunk
                del test_target_data_chunk
                del test_data_chunk
